...
export async function POST(req:forgotreq):Promise<any>{
    ...
    let randomcrypto =crypto.randomBytes(24).toString('hex');
    let restoken = crypto.createHash('sha256').update(randomcrypto).digest('hex');
    let Dates = Date.now() + 10 * 60 * 1000
   

        user.passResetToken = restoken
        user.passResetExp = Dates
        user.save()

   
    

    const reseturl = `http://${host}/api/user/resetpassword/${randomcrypto}`;
